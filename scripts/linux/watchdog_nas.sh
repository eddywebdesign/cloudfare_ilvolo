...
if ! tmux has-session -t trascrizione 2>/dev/null; then
  ...
  tmux new-session -d -s trascrizione -n 0 \
    "cd '$REPO' && source ~/ilvolo-env/bin/activate && export ILVOLO_AUDIO_ROOT='$ROOT' && bash scripts/linux/avvia_trascrizione_sicura.sh '' --skip-classify; bash"
  exit 0
fi
...
if ! pgrep -f "trascrivi_locale_episodi.py|whisperx" > /dev/null; then
  ...
  tmux send-keys -t trascrizione \
    "cd '$REPO' && source ~/ilvolo-env/bin/activate && export ILVOLO_AUDIO_ROOT='$ROOT' && bash scripts/linux/avvia_trascrizione_sicura.sh '' --skip-classify" Enter
fi
